...
a logical value indicating whether NA values should be snipped before the compilation proceeds.
...
There is a large range of values. Some with the thousands and others in the millions, using the median gives us the value for the middle attraction, where as the mean has been effected the results by some attractions receiving a significantly larger amount of visitors which has caused the mean to be inflated.
...
```{r Ex_m}
#not my code, copied from key to check for functionality - yet to complete myself
visitors_with_nations %>%
  group_by(nation) %>% 
  count(admission) %>%
